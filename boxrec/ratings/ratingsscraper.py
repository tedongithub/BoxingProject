...
from request import create_agents_list, random_header_agent, fetch_with_sem, silence_event_loop_closed
	...
	async def scrape(self, urls) -> list[str]:
		...
		async with aiohttp.ClientSession(connector=conn) as session:
			for count, url in enumerate(urls):
				page_num = await curr_page_num(self.config.base_url, self.config.base_href, url)
				headers = await random_header_agent(agents)

				try:
					page_content = await fetch_with_sem(sem, url, session, headers, page_num)
					if page_content.get('content_flag') == True:
						pages_content.append(page_content)
						log_msg(f"Successfully scraped URL: {count+1}/{len(urls)}... ratings page#: {page_num}")
					else:
