...
async def fetch(url, session, headers, boxrec_id=None) -> dict:
	...
	async with session.get(url, headers=headers) as response:
		...
		try:
			...
			if url+'?&offset=100' in response_text and url+'?&offset=200' in response_text:
				multipage_code = 2 
			elif url+'?&offset=100' in response_text and url+'?&offset=200' not in response_text:
				multipage_code = 1
			else: multipage_code = 0
