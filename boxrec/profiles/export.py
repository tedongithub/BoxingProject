...
async def export_scraped_content(collected_profiles: list[str]) -> None:  
	...
	for count, profile in enumerate(collected_profiles):
		...
		try:
			...
			log_msg(f"Saved {count+1}/{len(collected_profiles)} html files... page#: {boxrec_id} // output file: {output_file}")
		except:
			log_msg(f"id: {boxrec_id}, did not have the proper encoding and was skipped")
			continue 
...
async def save_all_profiles_to_csv(config: ScrapeConfig, all_profiles: list[BoxerProfile]) -> None:
...
async def save_all_fightsnips_to_csv(config: ScrapeConfig, all_fightsnips: list[list[FightSnippet]]) -> None:
