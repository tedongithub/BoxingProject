...
class ProfilesScraper:
	...
	async def scrape(self, urls: list[str]):
		...
		lst = await create_agents_list() ## NOTE: 'Might be good to store as class instance variable'
		...
		async with aiohttp.ClientSession(connector=conn) as session:
			for count, url in enumerate(urls):
				...
				try: 
					profile = await fetch_with_sem(sem, url, session, headers, br_boxer_id)
					...
					if profile.get('content_flag') == True:
						...
						log_msg(f"Successfully scraped br_boxer_id#: {br_boxer_id}... count: {count+1}/{len(urls)}")
					else:
						log_msg(f"\nFailed to scraped br_boxer_id#: {br_boxer_id}... count: {count+1}/{len(urls)}")
						break
				except:
					# raise Exception(f"\nProfile: {br_boxer_id}, {count+1}/{len(urls)} failed to create profile.")
