...
async def new_fight_snips_table() -> None:
    ...
    async with open_db(DRIVER) as cursor:
        ...
        create_table_command = """ 	
			IF OBJECT_ID(N'[dbo].[fightsnips]', N'U') IS NULL 
			BEGIN   
...
def query_saved_boxer_ratings_100(config: ScrapeConfig):
    ...
    with SQLServer(DRIVER) as cursor:
        cursor.execute(
            "SELECT TOP (100) [br_boxer_id] FROM [BoxingTestDB].[dbo].[ratings]"
        )
        a = cursor.fetchall()
...
def query_saved_boxer_careers(config: ScrapeConfig):
    ...
    with SQLServer(DRIVER) as cursor:
        cursor.execute(
            "SELECT DISTINCT([boxer_boxrec_id]) FROM [BoxingTestDB].[dbo].[fightsnips]"
        )
        a = cursor.fetchall()
...
def get_profiles_wins_100() -> list[str]:
    ...
    sql_statement = """	
		select 		br_boxer_id, boxer_bouts_tot
		from 		profiles 
