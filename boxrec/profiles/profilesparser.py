...
from transform import create_soup, clean_str, rekey, convert_to_float, convert_to_feet_float
...
class ProfilesParser:
	...
	def add_boxer_name(self, h1_tags: bs, profile_dict: dict) -> None:
		for tag in h1_tags:
			if tag.get('style') == "display:inline-block;margin-right:10px;line-height:30px;":
				profile_dict['boxer_name'] = tag.text.strip()

	def add_br_boxer_id(self, h2_tags: bs, profile_dict: dict) -> None:
		for tag in h2_tags:
			if tag.get('style') == "display:inline-block;line-height:100%;padding:0;margin:0;":
	...
	def add_boxer_ko_win_tot(self, profile_wld_table: bs, profile_dict: dict) -> None:
		if bool(profile_wld_table[0].find('th', {'class':'textWon'}).text):
			profile_dict['boxer_ko_win_tot'] = int(clean_str(profile_wld_table[0].find('th', {'class':'textWon'}).text.split('K')[0]))
	...
	def add_boxer_ko_loss_tot(self, profile_wld_table: bs, profile_dict: dict) -> None:
		if bool(profile_wld_table[0].find('th', {'class':'textLost'}).text):
			profile_dict['boxer_ko_loss_tot'] = int(clean_str(profile_wld_table[0].find('th', {'class':'textLost'}).text.split('K')[0]))
			
	def add_boxer_profile_attr(self, tags: list[bs], profile_dict: dict, attr: str) -> None:
		for tag in tags:

			if attr in tag.text:
				profile_dict[attr] = clean_str(tag.next_sibling.next_sibling.text)

	def add_boxer_titles_held(self, tags: list[bs], profile_dict: dict, attr: str = 'titles held') -> None:
		for tag in tags:
			if attr in tag.text and tag.next_sibling.next_sibling.find_all('a'):
				profile_dict['boxer_titles_held'] = [clean_str(t.text) for t in tag.next_sibling.next_sibling.find_all('a')]

	def add_boxer_height_ft(self, profile_dict: dict) -> None:
		try:
			profile_dict['boxer_height_ft'] = convert_to_feet_float(clean_str(profile_dict.get('boxer_height').split('/')[0]))
		except:
			pass

	def add_boxer_height_cm(self, profile_dict: dict) -> None:
		try:
			profile_dict['boxer_height_cm'] = int(clean_str(profile_dict.get('boxer_height').split('/')[1].split('cm')[0]))
		except:
			pass

	def add_boxer_reach_in(self, profile_dict: dict) -> None:
		try:
			profile_dict['boxer_reach_in'] = convert_to_feet_float(clean_str(profile_dict.get('boxer_reach').split('/')[0]))
		except:
			pass

	def add_boxer_reach_cm(self, profile_dict: dict) -> None:
		try:
			profile_dict['boxer_reach_cm'] = int(clean_str(profile_dict.get('boxer_reach').split('/')[1].split('cm')[0]))
		except:
			pass

	def add_boxer_wins_pct(self, profile_dict: dict) -> None:
		try:
			profile_dict['boxer_wins_pct'] = float(int(profile_dict.get('boxer_win_tot')) / int(profile_dict.get('boxer_bouts_tot')))
		except:
			pass

	def add_career_start_yr(self, profile_dict: dict) -> None:
		try:
			profile_dict['career_start_year'] = int(profile_dict.get('boxer_career_span').split('-')[0])	
		except:
			pass

	def add_career_end_yr(self, profile_dict: dict) -> None:
		try:
			profile_dict['career_end_year'] = int(profile_dict.get('boxer_career_span').split('-')[1])	
		except:
			pass
	...
	async def extract_profile(self, response_text: str) -> BoxerProfile:
		...
		if 'boxer_bouts_tot' in profile_dict: profile_dict['boxer_bouts_tot'] = int(profile_dict.get('boxer_bouts_tot'))
		if 'boxer_rounds_tot' in profile_dict: profile_dict['boxer_rounds_tot'] = int(profile_dict.get('boxer_rounds_tot'))
		if 'boxer_kos_pct' in profile_dict: profile_dict['boxer_kos_pct'] = float(profile_dict.get('boxer_kos_pct').replace('%',''))
		if 'boxer_debut_date' in profile_dict: profile_dict['boxer_debut_date'] = datetime.strptime(profile_dict.get('boxer_debut_date'), "%Y-%m-%d").date()
		if 'boxer_age' in profile_dict: profile_dict['boxer_age'] = int(profile_dict.get('boxer_age'))
		...
		# print(from_dict_to_dataclass(BoxerProfile, profile_dict), "\n")
			...
			if '-2' not in boxrec_id and '-1' not in boxrec_id:
				with open(file, 'r', encoding='utf-8') as f:
					...
					try:
						...
						log_msg(f"Profile Parser extracted data from html file: {count+1}/{len(list(files))}... boxer-ID#: {boxrec_id}")
					except:
						log_msg(f"Profile Parsing did NOT complete for file: {count+1}/{len(list(files))}... boxer-ID#: {boxrec_id}")
						continue
