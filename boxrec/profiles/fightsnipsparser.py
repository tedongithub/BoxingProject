...
class FightSnippetsParser:
	...
	def add_snip_fight_date(self, snippet: bs, snip_dict: dict) -> None:
		if len(snippet.find_all('a', href=re.compile('date'))) > 0:
			snip_dict['fsnip_fight_date'] = datetime.strptime(clean_str(snippet.find_all('a', href=re.compile('date'))[-1].text), "%Y-%m-%d").date()

	def add_snip_boxer_weighin_weight(self, snippet: bs, snip_dict: dict) -> None:
		if len(snippet.select("td:nth-child(3)")) == 1:
			try:
				snip_dict['fsnip_boxer_weighin_weight'] = convert_to_float(clean_str(snippet.select_one("td:nth-child(3)").text))
			except:
				pass

	def add_snip_opponent_name(self, snippet: bs, snip_dict: dict) -> None:
		if len(snippet.find_all('a', href=re.compile('proboxer'))) > 0:
			snip_dict['fsnip_opp_name'] = clean_str(snippet.find_all('a', href=re.compile('proboxer'))[-1].text)

	def add_snip_opponent_id(self, snippet: bs, snip_dict: dict) -> None:
		if len(snippet.find_all('a', href=re.compile('proboxer'))) > 0:
			snip_dict['fsnip_br_opp_id'] = int(clean_str(snippet.find_all('a', href=re.compile('proboxer'))[-1].get('href').split('r/')[1]))

	def add_snip_opponent_weighin_weight(self, snippet: bs, snip_dict: dict) -> None:
		if bool(snippet.find('a', href=re.compile('proboxer'))):
			try:
				snip_dict['fsnip_opp_weighin_weight'] = convert_to_float(clean_str(snippet.find('a', href=re.compile('proboxer')).parent.next_sibling.text))
			except:
				pass

	def add_snip_fight_result(self, snippet: bs, snip_dict: dict) -> None:
		if len(snippet.find_all('div', {'class':'boutResult'})) > 0:
			snip_dict['fsnip_fight_result'] = clean_str(snippet.find_all('div', {'class':'boutResult'})[0].text)

	def add_snip_fight_result_type(self, snippet: bs, snip_dict: dict) -> None:
		if len(snippet.find_all('div', {'class':'boutResult'})) > 0:
			# try:
			snip_dict['fsnip_fight_result_type'] = clean_str(snippet.find_all('div', {'class':'boutResult'})[0].parent.next_sibling.text)
			# except:
			# 	pass
	...
	def add_snip_fight_rounds_completed(self, snippet: bs, snip_dict: dict) -> None:
		if len(snippet.find_all('div', {'class':'boutResult'})) > 0:
			try:
				snip_dict['fsnip_fight_rounds_completed'] = int(clean_str(snippet.find_all('div', {'class':'boutResult'})[0].parent.next_sibling.next_sibling.text.split('/')[0]))
			except:
				pass				
	...
	def add_snip_fight_rounds_scheduled(self, snippet: bs, snip_dict: dict) -> None:
		if len(snippet.find_all('div', {'class':'boutResult'})) > 0:
			try:
				snip_dict['fsnip_fight_rounds_scheduled'] = int(clean_str(snippet.find_all('div', {'class':'boutResult'})[0].parent.next_sibling.next_sibling.text.split('/')[1]))
			except:
				pass

	def add_snip_event_fight_id(self, snippet: bs, snip_dict: dict) -> None:
		if len(snippet.find_all('a', href=re.compile('event'))) > 0:
			snip_dict['fsnip_event_fight_id'] = clean_str(snippet.find_all('a', href=re.compile('event'))[-1].get('href').split('t/')[1])

	# def add_snip_fight_stoppage_round_time(self, snip_bot: bs, snip_dict: dict) -> None:
	# 	if bool(snip_bot) and 'time:' in snip_bot.get_text():
	# 		snip_dict['snip_fight_stoppage_round_time'] = clean_str(snip_bot.get_text().split('time:')[1].split('|')[0])
	...
	async def extract_snip(self, snip_top: bs) -> FightSnippet:
		...
		# self.add_snip_fight_stoppage_round_time(snip_bot, snip_dict)

		# print(from_dict_to_dataclass(FightSnippet, snip_dict))
		...
		for count, file in enumerate(files):
			with open(file, 'r', encoding='utf-8') as f:
				...
				try:
					...
					log_msg(f"Fight Snips Parser extracted data from html file: {count+1}/{len(list(files))}... boxer-ID#: {boxrec_id}")
				except:
					log_msg(f"Parsing did NOT complete for fightsnip file: {count+1}/{len(list(files))}... boxer-ID#: {boxrec_id}")
					continue
