...
def clean_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["boxers_height_ft_l"] = df["boxers_height_ft_l"].fillna(0)
    df["boxers_height_ft_r"] = df["boxers_height_ft_r"].fillna(0)
    df["boxers_reach_in_l"] = df["boxers_reach_in_l"].fillna(0)
    df["boxers_reach_in_r"] = df["boxers_reach_in_r"].fillna(0)
    df["boxer_points_after_l"] = df["boxer_points_after_l"].fillna(0)
    df["boxer_points_after_r"] = df["boxer_points_after_r"].fillna(0)
...
async def fights_to_df(all_fights: list[Fight]) -> pd.DataFrame:
    ...
    return clean_df(pd.DataFrame(all_fights))
