...
from wrangle import create_soup, clean_str, curr_event_fight_id, convert_to_float, convert_to_feet_float
...
class FightParser:
    ...
    def add_fight_date(self, h2_tags: list[bs], fight_dict: dict) -> None:
        ...
        fight_dict['fight_date'] = datetime.strptime(
            fight_date, '%A %d, %B %Y').date()

    def add_fight_weightclass_and_rounds(self, tags: list[str], fight_dict: dict) -> None:
        for tag in tags:
            if 'contest' in tag.text.lower():
                fight_contest_type, fight_rounds_scheduled = clean_str(
                    tag.text).split(',')
    ...
    def add_referee_scores(self, table_top: bs, fight_dict: dict) -> None:
        if bool(table_top.find('a', href=re.compile('referee'))):
            referee_tag = table_top.find(
                'a', href=re.compile('referee')).parent
            try:
    ...
    def add_fight_judges_scores_l(self, fight_judges_tags: list[bs], fight_dict: dict) -> None:
        if len(fight_judges_tags) == 3:
            try:
                fight_dict['fight_judge1_score_l'] = int(
                    clean_str(fight_judges_tags[0].previous_sibling.previous_sibling.text))
                fight_dict['fight_judge2_score_l'] = int(
                    clean_str(fight_judges_tags[1].previous_sibling.previous_sibling.text))
                fight_dict['fight_judge3_score_l'] = int(
                    clean_str(fight_judges_tags[2].previous_sibling.previous_sibling.text))
            except:
                pass
    ...
    def add_fight_judges_scores_r(self, fight_judges_tags: list[bs], fight_dict: dict) -> None:
        if len(fight_judges_tags) == 3:
            try:
    ...
    def add_boxers_boxrec_ids(self, boxer_name_tag_l: bs, boxer_name_tag_r: bs, fight_dict: dict) -> None:
    ...
    def add_winner_loser_labels(self, boxer_name_tag_l: bs, boxer_name_tag_r: bs, fight_dict) -> None:
        ...
        if winner_flag != True:
            ...
            fight_dict['fight_loser_id'] = int(
                clean_str(boxer_name_tag_l.a.get('href').split('r/')[1]))
            fight_dict['fight_winner_name'] = clean_str(
                boxer_name_tag_r.a.text)
    ...
    def add_boxers_points_after(self, after_fight_tag: bs, fight_dict: dict) -> None:
        if bool(after_fight_tag.previous_sibling.previous_sibling.text):
            try:
                fight_dict['boxer_points_after_l'] = float(clean_str(
                    after_fight_tag.previous_sibling.previous_sibling.text.split('p')[0]))
            except:
                pass
            try:
                fight_dict['boxer_points_after_r'] = float(
                    clean_str(after_fight_tag.next_sibling.next_sibling.text.split('p')[0]))
            except:
                pass
    ...
    def add_boxers_wins_before(self, boxer_wins_before_tag: bs, fight_dict: dict) -> None:
    ...
    def add_boxers_losses_before(self, boxers_losses_before_tag: bs, fight_dict: dict) -> None:
    ...
    def add_boxers_draws_before(self, boxers_draws_before_tag: bs, fight_dict: dict) -> None:
    ...
    def add_boxers_kos_before(self, boxers_kos_before_tag: bs, fight_dict: dict) -> None:
    ...
    def add_fight_titles_avail(self, title_fight_flag_tag: bs, fight_dict: dict) -> None:
        if bool(title_fight_flag_tag) == True:
    ...
    def add_fight_rounds_completed(self, boxer_name_tag_l: bs, h2_tags: list[bs], fight_dict: dict) -> None:
    ...
    def add_boxers_reach_cm(self, boxer_reach_tag: bs, fight_dict: dict) -> None:
        try:
            fight_dict['boxer_reach_cm_l'] = int(clean_str(
                boxer_reach_tag.previous_sibling.previous_sibling.text.split('/')[1].split('cm')[0]))
            fight_dict['boxer_reach_cm_r'] = int(clean_str(
                boxer_reach_tag.next_sibling.next_sibling.text.split('/')[1].split('cm')[0]))
        except:
            pass

    def add_boxers_height_cm(self, boxer_height_tag: bs, fight_dict: dict) -> None:
        try:
            fight_dict['boxer_height_cm_l'] = int(clean_str(
                boxer_height_tag.previous_sibling.previous_sibling.text.split('/')[1].split('cm')[0]))
            fight_dict['boxer_height_cm_r'] = int(clean_str(
                boxer_height_tag.next_sibling.next_sibling.text.split('/')[1].split('cm')[0]))
        except:
            pass

    def add_boxers_reach_in(self, boxer_reach_tag: bs, fight_dict: dict) -> None:
        try:
            fight_dict['boxers_reach_in_l'] = convert_to_feet_float(clean_str(
    ...
    def add_boxers_height_ft(self, boxer_height_tag: bs, fight_dict: dict) -> None:
        try:
            fight_dict['boxers_height_ft_l'] = convert_to_feet_float(clean_str(
                boxer_height_tag.previous_sibling.previous_sibling.text.split('/')[0]))
        except:
            pass
        try:
    ...
    async def extract_fight(self, response_text: str) -> Fight:
        ...
        fight_judges_tags = [x.parent for x in table_top.find_all(
            'a', href=re.compile('judge'))]
        ...
        self.add_winner_loser_labels(
            boxer_name_tag_l, boxer_name_tag_r, fight_dict)
        self.add_boxers_points_after(after_fight_tag, fight_dict)
        self.add_boxers_boxrec_ids(
            boxer_name_tag_l, boxer_name_tag_r, fight_dict)
        ...
        # print(from_dict_to_dataclass(Fight, fight_dict))
    ...
    async def parse(self) -> list[Fight]:
        ...
        for count, file in enumerate(files):
            with open(file, 'r') as f:
                ...
                log_msg(f"Fight Parser extracted data from html file: {count+1}/{len(list(files))}... event_fight_id - ID#: {event_fight_id}")
                # except:
                #     log_msg(f"Fight Parsing did NOT complete for file: {count+1}/{len(list(files))}... event_fight_id - ID#: {event_fight_id}")
                #     continue
