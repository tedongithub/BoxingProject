...
DRIVER = 'DRIVER={ODBC Driver 17 for SQL Server};SERVER=' + \
	config.server+';DATABASE='+config.database+';TRUSTED_CONNECTION=yes'
...
def query_entire_fights_table():
	...
	with open_db(DRIVER) as cursor:
		cursor.execute(
			"SELECT * FROM [BoxingTestDB].[dbo].[fights]")
...
async def make_new_fights_table() -> None:
	...
	async with open_db(DRIVER) as cursor:
		...
		create_table_command = """  IF OBJECT_ID(N'[dbo].[fights]', N'U') IS NULL 
									BEGIN   
...
class SQLServer():
	...
	def query(self):
		...
		self.cnxn.cursor.execute(
			"SELECT * FROM [BoxingTestDB].[dbo].[profiles]")
...
def get_fight_ids_in_fights_table():
	...
	with SQLServer(DRIVER) as cursor:
		cursor.execute(
			"SELECT [event_fight_id] FROM [BoxingTestDB].[dbo].[fights]")
...
def get_fight_urls_not_in_db(config: ScrapeConfig) -> list:
	...
	return [build_next_url(config, x) for x in curr_careers_fights_urls if x not in curr_fights_fights_urls]
